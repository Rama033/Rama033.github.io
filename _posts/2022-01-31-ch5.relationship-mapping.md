...
- 서로 다른 둘을 mapping 하는데에는 다음 세 가지가 핵심 키워드
  - **Direction:** 참조하는 방향. 한 쪽만 참조하면 단방향, 서로 참조시 양방향(reference를 통한 relationship은 항상 단방향임. 서로 참조시에도 unidrectional relation이 두개인 것임). table은 항상 양방향
...
- object relationship
  ...
  - reference를 통한 relationship은 항상 단방향
- table relationship
...
## 5.3.1 양방향 relationship mapping
...
- entity의 relation 을 양방향으로 설정 시 객체 참조는 둘, foregin key는 하나
  - 차이가 발생
  - 따라서 owner를 정해야 함
...
## 5.4.1 양방향 mapping의 규칙: relationship의 owner
...
// member1 저장
Member member1 = new Member("member1", "Alice");
// relationship 설정: Member(Alice) -> Team(Alpha)
member1.setTeam(team1);
em.persist(member1);

// member2 저장
Member member2 = new Member("member2", "Bob");
// relationship 설정: Member(Bob) -> Team(Alpha)
member2.setTeam(team1);
em.persist(member2);
```
...
# 5.6 양방향 relationship 주의점
- 양방향 relationship 설정시 non-owning side 에만 값 넣으면 제대로 반영이 안됨
  - owner side 에서 mapping된 foreign key 관리하기 때문에 owner side에서 값 설정 안해주면 DB에 안들어감

<br/>
<br/>

## 5.6.1 순수한 객체까지 고려한 양방향 relationship
...
// relationship 설정: Member(Alice) -> Team(Alpha)
member1.setTeam(team1);
// relationship 설정: Team(Alpha) -> Member(Alice)
...
// relationship 설정: Member(Bob) -> Team(Alpha)
member2.setTeam(team1);
// relationship 설정: Team(Alpha) -> Member(Bob)
...
## 5.6.2 relationship 편의 method
- 위 예시처럼 양방향 relationship 에서는 양쪽 모두 값 넣어줘야 함
  - 직접 넣는 방식은 실수할 확률이 높음
- 따라서 한번에 양 쪽 다 넣어주는 method 작성해서 쓰는게 좋음
  - 이를 **relationship 편의 method**라 부름
...
public class Member {
  ...
  ...
  public void setTeam(Team team) {
    // 이미 다른 team과 relationship 있는 member일 수 있으므로 꼭 확인하고 있으면 관계 제거해야 함
    if (this.team != null) {
...
- relationship 편의 method 작성시 기존 relationship을 확인하고 제거해야 함
  - owner side에만 값 넣어주는 경우와 마찬가지로 이것도 persistence context 종료 후 새 context 에서 조회 시엔 정상적으로 작동하지만 그 전까지는 relationship 해제된 entity가 조회되기때문에 문제
